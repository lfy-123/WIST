...
class ActorPPOTrainer(ABC):
    def __init__(
        ...
        **kwargs,
        ...
        self.actor_loss_fn = PolicyLoss(
            ...
            generate_max_length=self.args.generate_max_len,
        )
    ...
    def ppo_train(self, kl_ctl: float):
        ...
        ROLE_NAME = {0: "challenger", 1: "reasoner"}
        ROLE_KEYS = ["policy_loss", "reward", "return", "response_length", "group_reward_std", "response_clip_ratio", "ppo_clip_ratio"]
        role_sum = {0: defaultdict(float), 1: defaultdict(float)}
        role_cnt = {0: 0.0, 1: 0.0}
...
# --- TODO: translate comment
                rid = experience.info.get("role_id", None)                   
# TODO: translate comment
                if hasattr(rid, "detach"):  # torch.Tensor
                    rid_list = rid.detach().cpu().tolist()
                elif isinstance(rid, list):
                    rid_list = [int(x) for x in rid]
                else:
                    rid_list = [int(rid)]  # TODO: translate comment

                c_n = sum(1 for x in rid_list if int(x) == 0)
                r_n = sum(1 for x in rid_list if int(x) == 1)

# status TODO: translate comment
                for k in ROLE_KEYS:
                    if k in status:
                        v = float(status[k])
                        if c_n > 0:
                        ...
                        if r_n > 0:
                            role_sum[1][k] += v * r_n
                role_cnt[0] += float(c_n)
                role_cnt[1] += float(r_n)
...
# ===== TODO: translate comment
        to_reduce = {
            "role/challenger/count": role_cnt[0],
            "role/reasoner/count": role_cnt[1],
        }
        for k, v in role_sum[0].items():
            to_reduce[f"role/challenger/sum/{k}"] = v
        for k, v in role_sum[1].items():
            to_reduce[f"role/reasoner/sum/{k}"] = v
        reduced = self.strategy.all_reduce(to_reduce, op="sum")  # TODO: translate comment
# all_reduce TODO: translate comment
        c_cnt = float(reduced["role/challenger/count"])
        r_cnt = float(reduced["role/reasoner/count"])
# ===== TODO: translate comment
# TODO: translate comment
        if status_mean is None or not isinstance(status_mean, dict):
            status_mean = {}
        status_mean["role/challenger/count"] = c_cnt
        status_mean["role/reasoner/count"] = r_cnt
        def _safe_mean(sum_val, cnt):
            return (sum_val / cnt) if cnt > 0 else 0.0
        for k in ROLE_KEYS:
            c_sum_key = f"role/challenger/sum/{k}"
            r_sum_key = f"role/reasoner/sum/{k}"
            if c_sum_key in reduced:
                status_mean[f"role/challenger/{k}"] = _safe_mean(float(reduced[c_sum_key]), c_cnt)
            if r_sum_key in reduced:
                status_mean[f"role/reasoner/{k}"] = _safe_mean(float(reduced[r_sum_key]), r_cnt)
        return status_mean
...
# states TODO: translate comment
                self.checkpoint_states = dict(states)
    ...
    def forward(
        ...
        device = torch.cuda.current_device()
        # logger.info(f"[Actor forward] device={device}, seq_shape={sequences.shape}, "
        #             f"action_mask_shape={getattr(action_mask, 'shape', None)}, "
        #             f"attn_mask_shape={getattr(attention_mask, 'shape', None)}")
