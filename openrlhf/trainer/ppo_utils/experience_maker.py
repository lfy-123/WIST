...
def update_samples_with_rewards(rewards_info, samples_list):
    ...
    print(f"[update_samples_with_rewards] samples_len: {samples_len}")

    rewards_list = torch.cat([torch.as_tensor(info["rewards"]) for info in rewards_info], dim=0).split(samples_len)
    print(f"[update_samples_with_rewards] rewards_list: {rewards_list}")
...
class SamplesGenerator:
    ...
    def _generate_vllm(self, all_prompts: List[str], all_labels, **kwargs) -> List[Experience]:
        ...
        print("\n===================== [DEBUG] ENTER _generate_vllm =====================")
        print(f"[DEBUG] #prompts before expand: {len(all_prompts)}")
        print(f"[DEBUG] #labels before expand:  {len(all_labels)}")
        ...
        print(f"[DEBUG] truncate_length = {truncate_length}")

        # Expand prompt list based on the number of samples per prompt
        n_samples_per_prompt = kwargs.pop("n_samples_per_prompt", args.n_samples_per_prompt)
        print(f"[DEBUG] n_samples_per_prompt = {n_samples_per_prompt}")
        ...
        print(f"[DEBUG] #prompts after expand = {len(all_prompts)} (original {original_prompt_count})")
        print(f"[DEBUG] #labels after expand  = {len(all_labels)}")
        print(f"[DEBUG] #tokenized prompts = {len(all_prompt_token_ids)}")
        
        # Distribute requests to engines and collect responses
        refs = []
        batch_size = (len(all_prompt_token_ids) + len(llms) - 1) // len(llms)
        
        print(f"[DEBUG] num_llms = {len(llms)}, batch_size per engine = {batch_size}")
        ...
        for i in range(len(all_outputs)):
            ...
            if i < 5:  # TODO: translate comment
                print(f"\n[Sample {i}]")
                print(f"  prompt 长度: {len(output.prompt_token_ids)}")
                print(f"  response_length: {response_length}")
                print(f"  sequences.shape: {sequences.shape}")
                print(f"  attention_mask.shape: {attention_mask.shape}")
                print(f"  action_mask.shape: {action_mask.shape}")
                print(f"  total_length (非 pad token 数): {total_length.item()}")
        ...
        print(f"\n生成的 Experience 数量: {len(samples_list)}")
        if samples_list:
            print(f"  第 0 个 samples.sequences.shape: {samples_list[0].sequences.shape}")
            print(f"  第 0 个 samples.attention_mask.shape: {samples_list[0].attention_mask.shape}")
            print(f"  第 0 个 samples.action_mask.shape: {samples_list[0].action_mask.shape}")
        ...
        if remote_reward_model:
            print("\n[get_rewards] 调用远程 RewardModel 计算奖励...")
            all_queries = sum(
                [
            ...
            print(f"  all_queries 数量: {len(all_queries)}")
            print(f"  all_prompts_flat 数量: {len(all_prompts)}")
            print(f"  all_labels_flat  数量: {len(all_labels)}")
            ...
            print(f"[get_rewards] rewards_info 类型: {type(rewards_info)}")
            if isinstance(rewards_info, list):
                print(f"[get_rewards] rewards_info 列表长度: {len(rewards_info)}")
                if rewards_info:
                    first = rewards_info[0]
                    print(f"[get_rewards] rewards_info[0] keys: {list(first.keys())}")
                    if "rewards" in first:
                        print(f"[get_rewards] rewards_info[0]['rewards'].shape: {torch.as_tensor(first['rewards']).shape}")
                    if "scores" in first:
                        print(f"[get_rewards] rewards_info[0]['scores'].shape: {torch.as_tensor(first['scores']).shape}")
                    if "extra_logs" in first:
                        print(f"[get_rewards] rewards_info[0]['extra_logs'] keys: {list(first['extra_logs'].keys())}")
            else:
                print("[get_rewards] WARNING: rewards_info 不是 list 类型，请检查 remote_reward_model.get_rewards 的返回值。")

            
            # Process rewards and scores
            update_samples_with_rewards(rewards_info, samples_list)
# TODO: translate comment
            print("\n[update_samples_with_rewards] 更新后的 samples_list 统计：")
            for i, s in enumerate(samples_list[:5]):
                if hasattr(s, "rewards") and s.rewards is not None:
                    print(f"  Sample {i}: rewards.shape = {s.rewards.shape}")
                if hasattr(s, "scores") and s.scores is not None:
                    print(f"              scores.shape  = {s.scores.shape}")
                if "reward" in s.info:
                    print(f"              info['reward'].shape = {s.info['reward'].shape}")
                if "score" in s.info:
                    print(f"              info['score'].shape  = {s.info['score'].shape}")

        print("==================== [_generate_vllm] END ====================\n")
        print(f"check lfy samples_list[0].rewards:{samples_list[0].rewards}")
        return samples_list
...
class RemoteExperienceMaker(ABC):
    ...
    def make_experience(self, samples_list: List[Experience]) -> List[Experience]:
        ...
        print(f"======================== 检查输入 ======================")
        print(f"samples_list[0].rewards:{samples_list[0].rewards}")
        ...
        if samples_list[0].rewards is not None:
            ...
            print(f"======================== 检查奖励 ======================")
            print(f"sequences_list[0]:{sequences_list[0].shape}")  # (B, S)
            print(f"remove_pad_token(seq, attention_mask):{len(remove_pad_token(sequences_list[0], attention_mask_list[0])[0])}, {len(remove_pad_token(sequences_list[0], attention_mask_list[0])[1])}")
            print(f"queries_list的长度:{len(queries_list)}")
        ...
        if samples_list[0].rewards is not None:
            ...
            # Process rewards and scores
            print(f"rewards_info长度:{len(rewards_info)}")
            print(f"check lfy rewards_info rewards:{rewards_info[0]['rewards'].shape}")
            print(f"check lfy rewards_info scores:{rewards_info[0]['scores'].shape}")
            print(f"check lfy samples_list长度:{len(samples_list)}")
            update_samples_with_rewards(rewards_info, samples_list)
        else:
            # Reward Model
