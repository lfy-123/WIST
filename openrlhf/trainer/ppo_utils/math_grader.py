...
def boxed_reward_fn(
    ...
    if model_answer is None:
        return model_answer, False  # Cannot even parse anything.
    ...
    if is_correct:
        return model_answer, True  # Correctness reward.
    else:
        return model_answer, False  # Formatted but wrong answer; no format reward to avoid hacking.



def boxed_reward_fn_for_eval(
    model_response, gt_answer, fast=False
):
    model_answer = extract_answer(model_response)
    if model_answer is None:
        if "\\boxed" in str(gt_answer):
            gt_answer = extract_answer(str(gt_answer))
        return model_answer, gt_answer, False  # Cannot even parse anything.
    if isinstance(gt_answer, float) or isinstance(gt_answer, int):
        gt_answer = str(gt_answer)
    if isinstance(gt_answer, str):
        is_correct = grade(model_answer, gt_answer, fast)
    ...
    if "\\boxed" in gt_answer:
        gt_answer = extract_answer(gt_answer)
    if is_correct:
        return model_answer, gt_answer, True  # Correctness reward.
    else:
        return model_answer, gt_answer, False  # Formatted but wrong answer; no format reward to avoid hacking.



def r1_distill_qwen_math_reward_fn(
    model_response,
    gt_answer,
    fast=False,
    correct_reward=1.0,
    ...
    if "</think>" in model_response and len(model_response.split("</think>")) == 2:
...
def answer_tag_reward_fn(
    ...
    if "</think> <answer>" in model_response and "</answer>" in model_response:
        ...
        if "\\boxed" in model_answer:
            ...
            if model_answer is None:
                return {"formatted": True}, incorrect_reward
        if isinstance(gt_answer, float) or isinstance(gt_answer, int):
            gt_answer = str(gt_answer)
        if isinstance(gt_answer, str):
            is_correct = grade(model_answer, gt_answer, fast)
            ...
            for gt in gt_answer:
                is_correct |= grade(model_answer, gt, fast)
        if is_correct:
            return {"formatted": True}, correct_reward  # Correctness reward.
        else:
            return (
                {"formatted": True},
                incorrect_reward,
