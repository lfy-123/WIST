...
def blending_datasets(
    ...
    dataset_split="train",
    data_type="",  # TODO: translate comment
    tokenizer=None,  # TODO: translate comment
    max_tokens_num=5298,  # TODO: translate comment
):
    ...
    for i, dataset in enumerate(datasets):
        ...
        # local dataset saved with `datasets.Dataset.save_to_disk`
            ...
            try:
                ...
                if "pubmed" in dataset:
                    ...
                    data = load_dataset(dataset, data_type, data_dir=data_dir)
                    strategy.print(f"loaded {dataset} from files")
        ...
        input_key = getattr(strategy.args, "input_key", None)
            
# ======== TODO: translate comment
        if tokenizer is not None and max_tokens_num is not None:
            kept_indices = []
            n_total = len(data)
...
# TODO: translate comment
                ...
                if len(input_ids) <= max_tokens_num:
                    kept_indices.append(idx)

# TODO: translate comment
                # if (idx + 1) % 10000 == 0 and strategy.is_rank_0():
                #     strategy.print(f"[blending_datasets] checked {idx+1}/{n_total}, kept {len(kept_indices)}")

            strategy.print(
                f"[blending_datasets] dataset={dataset} total={n_total}, "
                f"kept={len(kept_indices)} (<= {max_tokens_num} tokens)"
            )

            if kept_indices:
                data = data.select(kept_indices)
            else:
# TODO: translate comment
                data = data.select([])
        else:
