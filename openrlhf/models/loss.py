...
class PolicyLoss(nn.Module):
    ...
    def forward(
        ...
        if self.policy_loss_type == "ppo" or self.policy_loss_type == "dr_grpo":
...
# TODO: translate comment
        if action_mask is not None:
            mask = action_mask.bool()              # (B, T)
            rmin = ratio.masked_fill(~mask, 1e9).min(dim=-1).values   # (B,)
            rmax = ratio.masked_fill(~mask, -1e9).max(dim=-1).values  # (B,)
        else:
            rmin = ratio.min(dim=-1).values
            rmax = ratio.max(dim=-1).values
        for i in range(ratio.size(0)):
            print(f"[ratio] batch i={i} min={rmin[i].item():.6f} max={rmax[i].item():.6f}")
        ...
        vllm_kl = None
        if self.enable_vllm_is_correction and (self.policy_loss_type == "ppo" or self.policy_loss_type == "dr_grpo"):
        ...
        if self.policy_loss_type == "dr_grpo":
        ...
        clip_ratio = masked_mean(torch.lt(surr2, surr1).float(), action_mask, dim=None)  # TODO: translate comment
