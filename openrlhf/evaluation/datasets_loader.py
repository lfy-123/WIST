...
import random
from openrlhf.trainer.ppo_utils.math_grader import boxed_reward_fn_for_eval
...
class DatasetHandler(ABC):
    ...
    def get_score_custom(self, responses, answers):
        scores = []
        pred_answers = [] 
        gold_answers = []
        for r, a in zip(responses, answers):
            pa, ga, is_correct = boxed_reward_fn_for_eval(r, a)
            if is_correct:
                scores.append(1)
            else:
                scores.append(0)
            pred_answers.append(None if pa is None else str(pa))
            gold_answers.append(str(ga))

        acc = sum(scores) / len(scores) if scores else 0.0
        return scores, acc, pred_answers, gold_answers
...
class MathDatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        df = pandas.read_csv(
            "your_path_to_math_500/math_500_test.csv"
...
class Aime2025DatasetHandler(DatasetHandler):
    ...
    def load_data(self):
        dataset = load_dataset("MathArena/aime_2025", 'default')['train']
        examples = [row for row in dataset]
        questions = [example['problem'] for example in examples]*32
        answers = [example['answer'] for example in examples]*32
...
class GPQA_DiamondDatasetHandler(DatasetHandler):
    def __init__(self, answer_pattern: str = ANSWER_PATTERN_MULTICHOICE):
        super().__init__(answer_pattern)
...
def get_dataset_handler(dataset_name: str,name: str = None) -> DatasetHandler:
    if dataset_name == "math":
        ...
        return GPQA_DiamondDatasetHandler()
